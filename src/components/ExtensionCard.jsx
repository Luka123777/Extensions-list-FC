...
import { extensions as initialExtensions } from './ExtensonItems.jsx'; 
import {logos, Android12Switch} from './utils.jsx'
...
function ExtensionCardMain() {
  //cambio de estado con el swtich
 const [extensionList, setExtensionList] = useState(initialExtensions);

  const handleSwitchChange = (id) => (event) => {
    const updatedList = extensionList.map((ext) => {
      if (ext.id === id) {
        return { ...ext, estate: event.target.checked};
      } else {
        return ext;
      }
    });
  ...
  //Botones para filtrar extensiones activas y inactivas.
   const [filter, setFilter] = useState("all");

   const filteredExtensions = extensionList.filter((ext) => {
    if (filter === "active") {
      return ext.estate === true;
    }

    if (filter === "inactive") return ext.estate === false;

    return ext; // "all"
  });
  //Borrar extensiones
  const handleRemove = (id) => () => {
    const updatedList = extensionList.filter((ext) => ext.id !== id);
    setExtensionList(updatedList);
  };
  //Boton activo y cambio de color
   let[activeButton, setActiveButton] = useState(null);

   const cambiarColor = (buttonIndex) =>{
    setActiveButton(buttonIndex);
   }
   //DarkMode de la website
  ...
  return (
    <div className="content">
      ...
      <div className="presentation-title">
        ...
        <div className="buttons-group">
         ...
         <Button 
          ...
          variant="contained" onClick={() => {setFilter("active"); cambiarColor(2);}}>
          Active
         </Button>
          <Button 
          style={{
          ...
          variant="contained" onClick={() => {setFilter("inactive"); cambiarColor(3);}}>
          Inactive
         </Button>
        </div>
      </div>
      ...
      <div className="extensions-container">
        ...
        <div className="extContent" key={ext.id}>
          <div className="imageText">
            <ext.image className="imageText-image" width={50} height={50} />
            <div className="imageText-content">
              <Typography className="imageText-title" variant="h6">{ext.title}</Typography>
              <Typography className="imageText-text" variant="subtitle2">{ext.description}</Typography>
            </div>
          </div>
          <div className="extOptions">
            <Button style={{color: darkMode ? 'white' : 'rgb(33, 33, 70)'}}
            variant="outlined" onClick={handleRemove(ext.id)}>
              remove
            </Button>
            <Android12Switch checked={ext.estate} onChange={handleSwitchChange(ext.id)}/>
          </div>
        </div>
        ))}
      </div>
...
export default ExtensionCardMain;